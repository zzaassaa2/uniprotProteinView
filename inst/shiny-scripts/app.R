...
ui <- shinyUI(fluidPage(
  ...
  class = "card bg-light mb-3",
    ...
    p(class="card-text","Multiple proteins can be searched at once by leaving a space between entries. To specify color and \"|color\", but replacing the word color with your desired color ",
...
server <- function (input, output, session){
  ...
  observeEvent(input$addFile,{
    ...
    for(i in seq_along(spt)){
      s <- spt[[i]] #input
      xml <- uniprotProteinView::getProtein(s, FALSE)
      l <- length(xml)
      ...
      if(l > 0){
        ...
        for(i in seq_along(xml)){
          ...
          rv$files <- rbind(rv$files, list(protein = x[[1]], xml = x, features = f, colors = ifelse(input$fileChooseColor == "", uniprotProteinView::randomColor(), input$fileChooseColor)))
          insertUI(
